...
app.get('/testCommandePlat', async (req,res) => {
    try{
      const comm = await db.commande.findOne({where:{id_comm :1}, attributes:['id_comm']})    
      //   const co = comm.id_comm
      const pl = await db.Plat.findOne({where:{nom_palt:"Lasagnes"},attributes:['nom_palt']})
...
app.get('/testCommandeBoisson', async (req,res)=>{
    try{
        const comm = await db.commande.findOne({where:{id_comm :2}, attributes:['id_comm']})    
        const bo = await db.Boisson.findOne({ where:{ nom_boisson: 'coca'}, attributes:['nom_boisson']})
